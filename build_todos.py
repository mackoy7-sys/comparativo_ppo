...
# -*- coding: utf-8 -*-
"""
LE TODOS OS QUADRANTES (total e parcial) sem filtrar — materia-prima do
verificador. Nao grava no catalogo: serve para o verifica_reemb.py decidir,
por ANCORA no preco total ja validado, qual bloco e qual.

Duas mudancas sobre o build_set.py: o filtro de quadrante sai, e o dedupe
(cop,label,acom) passa a incluir o INDICE DO BLOCO — sem isso o segundo
quadrante seria descartado como repetido, que e exatamente o que mascarava
tudo.
...
D = '/Users/marcoscorrea/Downloads/'
...
def parse_price_tables(page, xlimit=99999):
    ...
    cur = None
    recent = []
    last_cop = None   # linha "COPARTICIPAÇÃO ..." do proprio quadro
    for r in rows:
        ...
        if deacc(first).upper().startswith('COPARTICIPACAO'):
            last_cop = deacc(t).upper().replace(' ', '')
        if first != 'PRODUTO':
            recent.append(t)
        if first == 'PRODUTO':
            hdr = deacc(''.join(recent[-3:])).upper().replace(' ', '')
            cur = {'prod_words': ws[1:], 'acoms': None, 'cols': None, 'prices': {},
                   'hdr': hdr, 'cop': last_cop}
            tables.append(cur)
                ...
                if len(groups) != len(spans):
                    ...
                    for ci, (code, cop) in enumerate(flat):
                        ...
                        out.append(dict(plano=plano, label=label + ' (PF)',
                                        acomodacao=acom, coparticipacao=cop,
                                        bloco=ti, hdr=tb.get('hdr',''), precos=pr))
            i = k; continue
        i += 1
    return out

def copart_do_bloco(tb, default):
    """A carga de 01/09 empilha ATE 4 quadros na MESMA pagina:
         copart parcial x reemb parcial | copart parcial x reemb total
         copart total   x reemb parcial | copart total   x reemb total
       Em agosto cada coparticipacao vinha em pagina propria, entao bastava um
       valor por pagina. Agora a coparticipacao tem de sair do cabecalho DO BLOCO."""
    # NAO usar o 'hdr' (janela de 3 linhas): ele engole o titulo de secao
    # "COM COPARTICIPAÇÃO TOTAL" que fica acima de um quadro que na verdade e
    # PARCIAL (visto na pag. de Mogi do SS 2-29) e inverte a leitura.
    cop = tb.get('cop') or ''
    if 'COPARTICIPACAOTOTAL' in cop: return 'Completa'
    if 'COPARTICIPACAOPARCIAL' in cop: return 'Parcial'
    return default

def entries_from_page(page, kind, praca, page_copart='Completa', pgi=-1, xlim=None):
    """kind: 'emp' (SS/PME) | 'ind' | 'amb'. A coparticipacao sai do cabecalho de
    cada bloco; o dedupe e por (copart, label, acomodacao) para nao descartar o
    segundo quadro da pagina."""
    out = []
    # 780 corta a caixa lateral "DESCONTO PROMOCIONAL POR TEMPO DETERMINADO".
    # MAS o Super Simples 1 Vida monta os dois quadros de reembolso LADO A LADO
    # na mesma tabela (10 colunas, a ultima em x=1024 numa pagina de 1191), e
    # o limite decepava justamente as colunas do reembolso parcial. La o limite
    # e solto; a ancora no catalogo continua sendo a rede de seguranca.
    xlimit = xlim if xlim else (780 if kind == 'emp' else 640)
    tables = parse_price_tables(page, xlimit=xlimit)
    if kind == 'emp':
        seen = set()
        for ti, tb in enumerate(tables):
            hdr = tb.get('hdr','')
            _cods = [c for c, _ in (tb.get('codes') or [])]
            tem900 = ('474.424/15-1' in _cods) or ('482.848/19-8' in _cods)
            # sem filtro: queremos TODOS os quadrantes
            cop = copart_do_bloco(tb, page_copart)
            codes = tb.get('codes') or []
            # Alinhamento POSICIONAL: dentro de um quadro, a linha REGISTRO ANS, a
            # linha ACOMODAÇÃO e cada linha de faixa têm o mesmo número de colunas e
            # na mesma ordem. Casar por índice é imune ao deslocamento de x causado
            # pela caixa lateral "DESCONTO PROMOCIONAL POR TEMPO DETERMINADO", que
            # fazia Enfermaria e Apartamento colapsarem no mesmo valor.
            acoms_tb = tb.get('acoms') or []
            alinhado = (len(codes) > 0 and len(codes) == len(acoms_tb)
                        and bool(tb['prices'])
                        and all(len(v) == len(acoms_tb) for v in tb['prices'].values()))
            if not codes:
                ...
                for prod, acom, ci in assign_columns(tb):
                    ...
                    out.append(dict(plano=plano, label=label, acomodacao=acom,
                                    coparticipacao=cop, bloco=ti, col=ci,
                                    pg=pgi, tem900=tem900, ncod=len(_cods),
                                    hdr=tb.get('hdr',''),
                                    precos=col_prices(tb, ci), alinhado=False))
                continue
            for idx, (code, cx) in enumerate(codes):
                info = ANS_EMP.get(code, 'MISS')
                if info == 'MISS':
                    print(f'  !! codigo ANS emp desconhecido {code} em {praca}'); continue
                if info is None: continue
                plano, label, acom = info
                if alinhado and acoms_tb[idx] != acom:
                    acom = acoms_tb[idx]   # a coluna manda (codigo repetido/typo no PDF)
                if not alinhado and tb.get('cols'):
                    # sem alinhamento posicional, a acomodacao da coluna manda
                    ai = min(range(len(tb['cols'])), key=lambda i: abs(tb['cols'][i] - cx))
                    if abs(tb['cols'][ai] - cx) < 55:
                        acom = tb['acoms'][ai]
                # Ancorar o preco na COLUNA de acomodacao, nao no x do codigo ANS.
                # A linha REGISTRO ANS pode vir deslocada quando a pagina tem caixa
                # lateral ("DESCONTO PROMOCIONAL POR TEMPO DETERMINADO"), e a escolha
                # por proximidade do codigo trocava Enfermaria por Apartamento no
                # Smart Prime (conferido em SP: Enf 328,26 / Apart 345,78).
                pr = {}
                bad = False
                if alinhado:
                    ...
                    ax = cx
                    if tb.get('cols'):
                        ai2 = min(range(len(tb['cols'])), key=lambda i: abs(tb['cols'][i] - cx))
                        if abs(tb['cols'][ai2] - cx) < 60:
                            ax = tb['cols'][ai2]
                    for faixa, vals in tb['prices'].items():
                        best = min(vals, key=lambda v: abs(v[1] - ax))
                        if abs(best[1] - ax) > 60: bad = True
                        pr[faixa] = best[0]
                if bad:
                    print(f'  !! {praca}: valores distantes p/ {label} {acom} — pulado'); continue
                out.append(dict(plano=plano, label=label, acomodacao=acom,
                                coparticipacao=cop, precos=pr, alinhado=alinhado,
                                bloco=ti, col=idx, pg=pgi, tem900=tem900,
                                ncod=len(_cods), hdr=tb.get('hdr','')))
    else:
        out = parse_ind_page(page, praca, kind == 'amb')
    return out

PRACA_MAP = {   # nomes do PDF -> nomes usados no catalog.json
    'São Paulo - SP': 'SP / RMSP',
    'São José dos Campos - SP': 'São José Dos Campos - SP',
}

def load_all():
    # Carga 08/09-30/09. Nao vieram os arquivos Individual (PF): pela regra do
    # usuario, "onde nao teve alteracao se mantem as tabelas anteriores".
    files = {
        'ss_demais': ('20260908 a 20260930 - Super Simples 2 a 29 vidas - Demais Empresas.pdf', 'emp', dict(mei=False, vmin=2, vmax=29, tipo='Empresarial')),
        'ss_mei':    ('20260908 a 20260930 - Super Simples 2 a 29 vidas - MEI.pdf', 'emp', dict(mei=True, vmin=2, vmax=29, tipo='Empresarial')),
        'pme_comp':  ('20260908 a 20260930 - PME - Compulsório v2.pdf', 'emp', dict(mei=False, vmin=30, vmax=99, tipo='Empresarial', contratacao='Compulsório')),
        'pme_adesao':('20260908 a 20260930 - PME - Adesão v2.pdf', 'emp', dict(mei=False, vmin=30, vmax=99, tipo='Empresarial', contratacao='Adesão')),
        'ss_1vida':  ('20260908 a 20260930 - Super Simples 1 Vida.pdf', 'emp', dict(mei=False, vmin=1, vmax=1, tipo='Empresarial')),
    }
    result = {}  # praca -> list of entries
    for tag, (fn, kind, meta) in files.items():
        pdf = pdfplumber.open(D + fn)
        seen_pages = {}
        for pgi, page in enumerate(pdf.pages):
            c = city_of(page)
            if not c: continue
            c = PRACA_MAP.get(c, c)
            ...
            if kind == 'emp':
                # ATENCAO: a carga de agosto trazia, por cidade, 1 pagina Parcial + 1
                # pagina Completa, e por isso valia a heuristica "1a=Parcial, 2a=Completa".
                # O pacote de 01/09 mudou de layout: cada cidade tem 2 paginas, AMBAS
                # "com coparticipacao parcial" (uma do bloco HMO, outra do bloco PPO), e
                # NAO ha nenhuma pagina Completa. Usar a ordem aqui rotularia Parcial
                # como Completa e gravaria preco errado. Entao: confiar no cabecalho.
                n = seen_pages.get(c, 0); seen_pages[c] = n + 1
                page_copart = hdr_copart
            else:
                page_copart = hdr_copart
            ents = entries_from_page(page, kind, c, page_copart, pgi=pgi,
                                     xlim=(99999 if tag == 'ss_1vida' else None))
...
def guarda_completa(result):
    """Trava de seguranca: nao deixar gravar um catalogo pela metade.
    Se uma praca vier so com Parcial, o merge deixaria a Completa de agosto no lugar
    e o vendedor nao teria como saber que esta olhando duas tabelas diferentes."""
    faltando = []
    for praca, ents in result.items():
        emp = [e for e in ents if e.get('tipo') == 'Empresarial']
        if emp and not any('Completa' in (e.get('coparticipacao') or '') for e in emp):
            faltando.append(praca)
    if faltando:
        print('\n' + '!' * 78)
        print('!! PACOTE INCOMPLETO — nao gravar no catalog.json')
        print('!! Sem tabela "com coparticipacao completa/total" em %d praca(s):' % len(faltando))
        for p in sorted(faltando):
            print('!!   -', p)
        print('!! Pedir os PDFs da versao Completa antes de seguir.')
        print('!' * 78)
    return not faltando

if __name__ == '__main__':
    result = load_all()
    guarda_completa(result)
    json.dump(result, open('/tmp/todos_parsed.json', 'w'), ensure_ascii=False, indent=1)
